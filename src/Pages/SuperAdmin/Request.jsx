...
import { RequestGetApi } from 'src/Utils/Apis'
import { RequestDeleteApi } from 'src/Utils/Apis'
import { RequestGetByIdApi } from 'src/Utils/Apis'
import { RequestPutApi } from 'src/Utils/Apis'
import { RequestUpdatePutApi } from 'src/Utils/Apis'
...
const ManageFaq = () => {
  ...
  // // console.log(reqDate,'my dateeee')
  ...
  const GetByIdApi = async (id) => {
    ...
    try {
      ...
      setReqphone(response?.data?.data?.contactNo)
      setReqstatus(response.data?.data?.status)
      setReqemail(response.data?.data?.email)
      // setReqmsg(response.data.reqMsg)
      setReqid(response.data.id)
      setReqDate(response.data?.data?.registrationTime?.slice(0, 10) || "");
      setLoader(false)
  ...
  const requestUpdateApi = async () => {
    ...
    if (!isValidPassword === false) {
      ...
      try {
        ...
        formData.append('contactNo', reqphone)
        formData.append('id', forUpdate)
          ...
          <div className='d-flex g-1 for-media-query'>
            <form class="row  me-2">
              ...
              <div className="col-3 p-0 ps-2" >
                <Link type="submit" className="btn btn-primary mb-3 heading-1 remove-shadow button-bg-color heading-14" style={{ border: '1px solid #008479', lineHeight: '1.3', display: 'flex' }} onClick={showName}>
                  ...
                  <p className='mb-0' style={{ fontSize: '14px' }}>Filter</p>
                </Link>
              </div>
            ...
            <div className='me-2'>
              <div class="input-group mb-3 ">
                <input type="text" class="form-control form-focus font-color" style={{ height: '34px' }} placeholder="Search" aria-label="Recipient's username" aria-describedby="basic-addon2" onChange={handleChange} value={searchKey} />
                  ...
                  data && data.length > 0 ? (
                    data?.map((item, index) => (
                      <tr key={item.id} className='heading-14 my-bg-color  align-middle'>
                        ...
                        <td className='greyText'>{item?.registrationTime ? item.registrationTime.slice(0, 10) : ''} </td>
                        <td className='greyText'>{item.email}</td>
                        <td className='greyText'>{item.contactNo}</td>
                        {/* <td className=''><p className={`${item.status === 'IN_PROGRESS' ? 'inProgressStatus' : `${item.status === 'REJECTED' ? 'rejectedStatus' : 'completeStatus' }``}>{item.status}</p></td> */}
                        <td className=''>
                          <p className={
                            `${item.status === 'IN_PROGRESS' ? 'inProgressStatus' :
                              item.status === 'REJECTED' ? 'rejectedStatus' :
                                'completeStatus'}`
                          }>
                            {item.status}
                          </p>
                        ...
                        <td className='greyText'>
                          <div class="dropdown my-button-show">
                            ...
                            <ul class="dropdown-menu heading-14 anchor-color heading-14" >
                              <li><Link class="dropdown-item" data-bs-toggle="offcanvas" data-bs-target="#staticBackdrop" aria-controls="staticBackdrop" onClick={(e) => GetByIdApi(item.id)}>Edit Request</Link></li>
                              {/* <li><Link class="dropdown-item" data-bs-toggle="offcanvas" data-bs-target="#staticBackdrop9090" aria-controls="staticBackdrop" onClick={(e) => GetByIdApi(item.id)}>View Details</Link></li> */}
                              {/* <li><a class="dropdown-item" href="#" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight2233" aria-controls="offcanvasRight2233" onClick={() => delValueUpdate(item.id)}>Delete</a></li> */}
                            </ul>
                          </div>
                        </td>
                      </tr>
                        ...
                        (
                          <>
                            <div class="mb-3 my-div-class">
                              ...
                              <p className='heading-14'>{reqmsg} <br /> <span><Link href="" onClick={() => { Replyhandle() }}>Reply</Link></span>
                              </p>
            ...
            reply && (
              <>
                ...
                <div className="offcanvas-body">
                  ...
                  <div class="mb-3 my-div-class">
                    <label for="exampleFormControlTextarea1" class="form-label label-color">Req. Message</label>
                    <p className='heading-14'>{reqmsg}<textarea class="form-control form-focus heading-14 mt-3" id="exampleFormControlTextarea1" value={datamsg} onChange={(e) => setDatamsg(e.target.value)} rows="3" placeholder='Reply here....' ></textarea>
