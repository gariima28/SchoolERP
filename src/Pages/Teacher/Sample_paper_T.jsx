...
import { TeacherSamplePaperPostApi } from 'src/Utils/Apis'
...
const Sample_paper = () => {
    ...
    const handleDownloadPdf = async (id) => {
        // console.log('my pdf iddddd', id)
        try {
            const response = await TeacherSamplePDF(id);
            if (response?.status === 200 && response?.data?.status === 'success') {
                toast.success(response?.data?.message);

                const pdfData = response?.data?.pdf; // Get PDF data from the response
                const blob = base64ToBlob(pdfData, 'application/pdf'); // Convert base64 to Blob
                        ...
                        <table className="table table-sm table-striped">
                            <thead className=''>
                                <tr className='heading-16 text-color-000' style={{ fontWeight: '500' }} >
                                    <th className='no-wrap' style={{ width: '6%' }}>#</th>
                                    <th className='no-wrap' style={{ width: '8%' }}>Title</th>
                                    <th className='no-wrap' style={{ width: '8%' }}>Class</th>
                                    <th className='no-wrap' style={{ width: '8%' }} >Section</th>
                                    <th className='no-wrap' style={{ width: '8%' }}>Subject</th>
                                    <th className='no-wrap' style={{ width: '8%' }} >Sample Paper Details</th>
                                    <th className='no-wrap' style={{ width: '8%' }} >Action</th>
                                </tr>
                            </thead>
                            <tbody className='heading-14 align-middle greyTextColor'>
                                {
                                    sampleData?.map((item, index) => (
                                        <tr className='heading-14' key={index} >
                                            ...
                                            <td className=' greyText no-wrap' >{item.title}</td>
                                            <td className=' greyText no-wrap' >{item.classNo}</td>
                                            <td className=' greyText no-wrap' >{item.sectionName}</td>
                                            <td className=' greyText no-wrap' >{item.subjectName}</td>
                                            <td className=' greyText d-flex no-wrap' >
                                                <p  >
