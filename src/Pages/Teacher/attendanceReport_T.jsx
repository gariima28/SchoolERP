...
// import MyStopwatch from './MyTime_T'
import HashLoader from 'src/Pages/HashLoaderCom';
import { useNavigate } from 'react-router-dom';
import MyTime from './MyTime_T';
...
const AssignLeave = () => {
  ...
  useEffect(() => {
    if (view && startWeekDate && endWeekDate || monthYear) {
      MyLeaveGetAllApi(view);
    }
  }, [view, startWeekDate, endWeekDate, pageNo, monthYear]);

  // Leave Get All Api   
  const MyLeaveGetAllApi = async (data) => {
    setView(data);
    setLoader(true);
    const startDate = startWeekDate;
    const formattedStartDate = new Date(startDate).toISOString().split('T')[0];
    const endDate = endWeekDate;
    const formattedEndDate = new Date(endDate).toISOString().split('T')[0];
    try {
      const response = await TeacherAttendanceTeacherGetAllApi(pageNo, pageSize, formattedStartDate, formattedEndDate, formattedDate, view);
      console.log('Attendance teacher get all responseeee--', response);
  ...
  const formattedDate = funct(dateStr);
    ...
    <Container>
      ...
      <div className="container-fluid main-body p-3">
        ...
        <div className='d-flex justify-content-between for-dislay-direction'>
          ...
          <div className='calendar-header mrgn-lft'>
            {view === 'month' ? (
              <div>
          ...
          <div className='d-flex g-1 for-media-query mrgn-lft' >
            <div className='pe-2'  >
              <button type="button" className="btn my-btn12 heading-12  mt-1" data-bs-dismiss="offcanvas" onClick={() => (MyLeaveGetAllApi('week'), setPageNo(1))} >Week View</button>
            </div>
            <div className='pe-2'>
              <button
                type="button"
                className="btn my-btn12 heading-12 mt-1"
                data-bs-dismiss="offcanvas"
                onClick={() => MyLeaveGetAllApi('month')}>
                Month View
              </button>
            </div>
            <div className="form-check form-switch pe-2 pt-1">
              <input
                style={{ width: '60px', height: '25px' }}
                className="form-check-input My-own-form-check-input"
                checked={isChecked}
            ...
            <table className="table table-sm table-striped">
              <thead className=''>
                <tr className='heading-16 text-color-000 ' style={{ fontWeight: '500' }}>
                  {/* <th className=' pe-0 no-wrap' style={{ width: '15%' }} >#</th> */}
                  <th className=' pe-0 no-wrap' style={{ width: '15%' }} >Date</th>
                  <th className='no-wrap' style={{ width: '8%' }}>Check In</th>
                  <th className='no-wrap' style={{ width: '300%' }}></th>
                  <th className='no-wrap' style={{ width: '15%' }}>Check Out</th>
                  <th className='no-wrap' style={{ width: '15%' }}>Total Hours</th>
                </tr>
              </thead>
              <tbody className='heading-14 align-middle greyTextColor greyText'>
                {
                  attendanceTeacherData && attendanceTeacherData?.length > 0 ? (
                    attendanceTeacherData?.map((item, index) => (
                      <tr className='heading-14' key={index}>
                        ...
                        <td className=' greyText no-wrap pe-0'>{item.date}</td>
                        {/* <td className=' greyText no-wrap pe-0'>{item.checkIn ? item.checkIn : 'Nt-chkng'} AM</td> */}
                        <td className='greyText no-wrap pe-0'>
                          {item?.checkIn ? (
                            new Date(`1970-01-01T${item.checkIn}`).toLocaleTimeString('en-US', {
                              hour: '2-digit',
                              minute: '2-digit',
                              hour12: true
                            })
                        ...
                        <td className='greyText no-wrap ps-3'>
                          <div className='pt-1 progress-dot-container'>
                            <div
                            ...
                            >
                              ...
                              {/* Small moving dots */}
                              {item?.hourPercent != null && item?.isCheckedIn &&
                                [...Array(10)].map((_, i) => (
                                  <div
                                    key={i}
                                    className="small-moving-dot"
                                    style={{
                                      position: 'absolute',
                                      left: '0%',
                                      top: '50%',
                                      ...
                                      borderRadius: '50%',
                                      backgroundColor: '#4CAF50',
                                      animation: `moveDot 4s infinite linear ${i * 0.5}s`,
                                      opacity: 0.9,
                                      zIndex: 2
                                    }}
                                  ></div>
                              ...
                              {/* Main progress dot */}
                              <div
                                className="main-progress-dot"
                                style={{
                                  position: 'absolute',
                                  left: `${item.hourPercent || 0}%`,
                                  top: '50%',
                        ...
                        {/* <td className=' greyText pe-0 no-wrap'>{item.checkOut ? item.checkOut : 'Nt-ChkOut'} PM</td> */}
                        <td className='greyText no-wrap pe-0'>
                          {item?.checkOut ? (
                            new Date(`1970-01-01T${item.checkOut}`).toLocaleTimeString('en-US', {
                              hour: '2-digit',
                              minute: '2-digit',
                              hour12: true
                            })
                        ...
                        <td className=' greyText pe-0 no-wrap'>{`${item.totalHour?.hours ? item.totalHour?.hours : 0} : ${item.totalHour?.minutes ? item.totalHour?.minutes : 0} : ${item.totalHour?.seconds ? item.totalHour?.seconds : 0} `}</td>
                      </tr>
                    ))
                  )
                    :
