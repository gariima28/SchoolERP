...
const Assign_publish = (props) => {
  ...
  useEffect(() => {
    ...
    navigate('/teacher/assignmenttea', { state: dataToSend });
  }, [pageNo])
  ...
  const assignments = props.data?.assignment ? props.data?.assignment : [];
  ...
  useEffect(() => {
    UpdatClassGetApi()
    MySectionGetApi(classId)
    MySubjectByClassIdGetApi(classId)
  }, [classId])
  ...
  const MyAssigmntGetByIdGetAll = async (id) => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        setClassNo(response?.data?.assignment?.classNo)
