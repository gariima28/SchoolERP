...
import { TeacherOnlinePostApi } from 'src/Utils/Apis'
...
import { TeacherOnlineCourseGetAllApi } from 'src/Utils/Apis'
import { TeacherOnlineDeleteApi } from 'src/Utils/Apis'
import { TeacherSubjectByClassIdInSyllabusGetAllApi } from 'src/Utils/Apis'
import { TeacherOnlineGetById } from 'src/Utils/Apis'
import { TeacherOnlinePutApi } from 'src/Utils/Apis'
...
const OnlineCourse = () => {
  ...
  const [swicthInput22, setSwicthInput22] = useState(false)
  ...
  const FuncValidation = () => {
    ...
    // lesson
    if (!lesson || lesson === "" || !/^\d+$/.test(lesson)) {
  ...
  const MyOnlinePostApi = async () => {
    ...
    if (FuncValidation()) {
      ...
      setLoader(true)
      try {
        const response = await TeacherOnlinePostApi(formData);
  ...
  const MyOnlineGetById = async (id) => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        setCourseImage(response?.data?.course?.courseImage)
        setCourseFile(response?.data?.course?.courseFile)
      ...
      <div className="container-fluid main-body p-3">
        ...
        <div className='d-flex justify-content-between for-dislay-direction'>
          ...
          <div className='d-flex g-1 for-media-query'>
            ...
            <Link type="button" className="btn btn-success heading-16 my-own-button me-3" data-bs-toggle="offcanvas" data-bs-target="#staticBackdrop" aria-controls="staticBackdrop" to={''} onClick={ClearData}>+ Add New Courses</Link>
          </div>
        ...
        <div className="main-content-conatainer pt-1 ">
          ...
          <div className="table-container px-3 table-responsive">
            <div>
              ...
              <div className="row mt-3 flex-direction">

                <div className="col-lg-2  col-md-6 col-sm-6 mrgn-btm">
                  ...
                  onlineAllData && onlineAllData?.length > 0 ? (
                    onlineAllData?.map((item, index) => (
                      <tr className='heading-14' >
                        ...
                        <td className=' no-wrap  greyText'>{item.classId}</td>
                        <td className=' no-wrap greyText'>
                          <span className=' pt-1'>Total section: {item.courseSection}</span> <br />
                          <span>Total lesson: {item.lesson}</span>
                        </td>
                        <td className='greyText greenText no-wrap'>{`${item.status ? 'Active' : 'Inactive'}`}</td>
              ...
              <div className="offcanvas-end offcanvas" data-bs-backdrop="static" tabindex="-1" id="staticBackdrop" aria-labelledby="staticBackdropLabel" ref={offcanvasRef}>
                ...
                <div class="offcanvas-body pt-0">
                  <div className="input " >
                    ...
                    <div class="mb-3">
                      <label for="exampleFormControlInput1" class="form-label label-color heading-16">Upload Course Image</label>
                      <input type="file" class="form-control form-control-sm" id="exampleFormControlInput1" onChange={(e) => setCourseImage(e.target.files[0])} placeholder="Select Class" accept='.jpg, .png, .jpeg' />
                    </div>
                    <div class="mb-3">
                      <label for="exampleFormControlInput1" class="form-label label-color heading-16">Upload Course File</label>
                      <input type="file" class="form-control form-control-sm" id="exampleFormControlInput1" onChange={(e) => setCourseFile(e.target.files[0])} placeholder="Select Class" accept='.pdf' />
                    </div>
                    <div className="mb-1">
                ...
                <div class="offcanvas-body pt-0">
                  <div className="input " >
                    ...
                    <div class="mb-3 " style={{ display: 'flex', }}>
                      <div className='w-100'>
                        <label for="exampleFormControlInput1" class="form-label label-color">Upload Course Image</label>
                        {
                          courseImage && swicthInput ?
                            <input type="file" class="form-control" id="exampleFormControlInput1" onChange={(e) => setCourseImage(e.target.files[0])} placeholder="select file" accept='.jpg, .png, .jpeg' />
                            :
                            <input type="text" class="form-control" id="exampleFormControlInput1" value={courseImage} placeholder="name@example.com" />
                        }
                      </div>
                    ...
                    <div class="mb-3 " style={{ display: 'flex', }}>
                      <div className='w-100'>
                        <label for="exampleFormControlInput1" class="form-label label-color">Upload Course File</label>
                        {
                          courseFile && swicthInput22 ?
                            <input type="file" class="form-control" id="exampleFormControlInput1" onChange={(e) => setCourseFile(e.target.files[0])} placeholder="select file" accept='.pdf' />
                            :
                            <input type="text" class="form-control" id="exampleFormControlInput1" value={courseFile} placeholder="name@example.com" />
                        }
                      </div>
                      <div style={{ margin: 'auto', paddingTop: '30px', paddingLeft: '5px' }}>
                        <button type="button" class="btn btn-outline-success my-green heading-14 " onClick={() => setSwicthInput22(!swicthInput22)} >{swicthInput22 ? "View " : "Edit"}</button>
                      </div>
