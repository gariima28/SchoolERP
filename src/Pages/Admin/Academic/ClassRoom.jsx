...
import { ClassRoomPostApi } from 'src/Utils/Apis'
import { ClassRoomGetApi } from 'src/Utils/Apis'
import { ClassRoomGetByIdApi } from 'src/Utils/Apis'
import { classRoomDeleteApi } from 'src/Utils/Apis'
import { ClassRoomPutApi } from 'src/Utils/Apis'
...
const ClassRoom = () => {
  ...
  const FuncValidation = () => {
    ...
    setLoader(false)
    // name 
    if (!addclassroom || addclassroom === "" || !/^[a-zA-Z0-9!@#$%^&*()_+=-]+$/.test(addclassroom)) {
  ...
  const handleClassNo = (e2) => {
    setAddclassroom(e2);
    const noRegex = /^[a-zA-Z0-9!@#$%^&*()_+=-]+$/;    setIsValidNameRequired(noRegex.test(e2));
            ...
            <table className="table table-sm table-striped">
              <thead className=''>
                <tr className='heading-16 text-color-000' style={{ fontWeight: '500' }}>
                  ...
                  <th className='no-wrap' style={{ width: '75%' }}>Class Room</th>
                  ...
                  classroomdata && classroomdata?.length > 0 ? (
                    classroomdata?.map((item, index) => (
                      <tr className='heading-14'  >
                        ...
                        <td className=' greyText no-wrap'>{item.roomNo}</td>
