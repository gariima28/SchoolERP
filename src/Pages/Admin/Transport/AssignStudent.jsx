...
import { DownloadAssignStudentsCsv, DownloadVehicleCsv } from '../../../Utils/Apis';
...
const AssignStudent = () => {
    ...
    useEffect(() => {
        ...
        DownloadCSV();
        DownloadPDF();
    }, [token, currentPage, pageNo, AssignStudentState])
        ...
        try {
            ...
            if (response?.status === 200) {
                if (response?.data?.status === 'success') {
                    ...
                    // console.log('inside', AssignStudentState)
                    ...
                    <div className="row pb-3 gap-xl-0 gap-3">
                        ...
                        <div className="col-xxl-8 col-xl-9 col-lg-12 col-sm-12 pe-0">
                            <ActionControls
                                ...
                                exportCSVAction={DownloadVehicleCsv}
                                    ...
                                    <div className="row gap-md-0 gap-sm-3">
                                        <div className="col-md-8 col-sm-12 col-8 text-sm-end text-start ps-0">
                                            <form className="d-flex" role="search">
                                                <input className="form-control formcontrolsearch font14" type="search" disabled={AssignStudentData.length > 0 ? false : true} placeholder="Search" aria-label="Search" onChange={(e) => setSearchByKey(e.target.value)} />
                                                <button className="btn searchhhButtons text-white " type="button" disabled={AssignStudentData.length > 0 ? false : true}><span className='font14' onClick={getAllAssignStudentData}>Search</span></button>
                                            </form>
                                        </div>
                                        ...
                                        <table className="table align-middle table-striped">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th className='textWrapClass'><span className='font14'>Driver No</span></th>
                                                    <th className='textWrapClass'><span className='font14'>Student Details</span></th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {AssignStudentData.map((item, index) => (
                                                    <tr key={item.vehicleId} className='my-bg-color align-middle'>
                                                        ...
                                                        <td className='textWrapClass greyText'><h3>{item.vehicleNo}</h3></td>
                                                        <td className='textWrapClass greyText'><h3>{item.route}</h3></td>
                                                        <td className='textWrapClass greyText'><h3>{item.driverName}</h3></td>
                                                        <td className='textWrapClass greyText'><h3>{item.driverNo}</h3></td>
                                                        <td className='textWrapClass greyText'><h3 className='align-self-center pointer' data-bs-toggle="modal" data-bs-target="#StudentDetailsModal" onClick={() => setAssignStudentTableData(item.students)}>Details <Icon icon="material-symbols:info-outline" width="1.4em" height="1.4em" style={{ color: '#008479' }} /></h3></td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
