...
//    const { userId } = useContext(MyUseContext);
   const myUserID = userId ?? roleId ?? "";
    ...
    const [isValidFaceBookUrl, setIsValidFaceBookUrl] = useState(false);
    const [isValidLinkedInUrl, setIsValidLinkedInUrl] = useState(false);
    const [isValidTwitterUrl, setIsValidTwitterUrl] = useState(false);
    const [isValidInstagramUrl, setIsValidInstagramUrl] = useState(false);
    const [isValidGooglePlus, setIsValidGooglePlus] = useState(false);

    const urlRegex = /^(https?:\/\/)?([\w\d-]+\.)+[\w\d]{2,}(\/.*)?$/;

    const validateField = (value, setIsValid) => {
        if (!value || value.trim() === '') {
            setIsValid(true);
            return false;
        }
    ...
    const handleFaceBookUrl = (e) => setFaceBookUrl(e.target.value);
    const handleLinkedInUrl = (e) => setLinkedInUrl(e.target.value);
    const handleTwitterUrl = (e) => setTwitterUrl(e.target.value);
    const handleInstagramUrl = (e) => setInstagramUrl(e.target.value);
    const handleGooglePlus = (e) => setGooglePlus(e.target.value);

    const FuncValidation = () => {
        const isFaceBookValid = validateField(faceBookUrl, setIsValidFaceBookUrl);
        const isLinkedInValid = validateField(linkedInUrl, setIsValidLinkedInUrl);
        const isTwitterValid = validateField(twitterUrl, setIsValidTwitterUrl);
        const isInstagramValid = validateField(instagramUrl, setIsValidInstagramUrl);
        const isGooglePlusValid = validateField(googlePlus, setIsValidGooglePlus);

        return isFaceBookValid && isLinkedInValid && isTwitterValid && isInstagramValid && isGooglePlusValid;
    };

    const ContactDataApi = async () => {
        if (!FuncValidation()) {
            toast.error('Please fill all fields with valid URLs');
            return;
        }
            ...
            <div className="form-container">
                ...
                <div className="row px-3 mt-4">
                    <div className="col-lg-6 col-md-6 col-sm-12">
                        <label htmlFor="facebook" className="form-label mb-0 heading-14 label-color">Facebook <span style={{ color: 'red' }}>*</span></label>
                        ...
                        <label htmlFor="twitter" className="form-label mb-0 heading-14 label-color">Twitter <span style={{ color: 'red' }}>*</span></label>
                        <div className="input-group mb-3">
                            <span className="input-group-text p-0" id="twitter-addon">
                                <svg width="37" height="37" viewBox="0 0 37 37" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <label htmlFor="instagram" className="form-label mb-0 heading-14 label-color">Instagram <span style={{ color: 'red' }}>*</span></label>
                        <div className="input-group mb-3">
                            <span className="input-group-text p-0" id="instagram-addon">
                                <svg width="37" height="37" viewBox="0 0 37 37" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g clip-path="url(#clip0_5557_2600)">
                                        <path d="M37 34.6875C37 35.964 35.964 37 34.6875 37H2.3125C1.036 37 0 35.964 0 34.6875V2.3125C0 1.036 1.036 0 2.3125 0H34.6875C35.964 0 37 1.036 37 2.3125V34.6875Z" fill="url(#paint0_linear_5557_2600)" />
                                        <path d="M25.4375 5.78125H11.5625C8.4975 5.78125 5.78125 8.4975 5.78125 11.5625V25.4375C5.78125 28.5025 8.4975 31.2188 11.5625 31.2188H25.4375C28.5025 31.2188 31.2188 28.5025 31.2188 25.4375V11.5625C31.2188 8.4975 28.5025 5.78125 25.4375 5.78125ZM18.5 25.4375C15.4348 25.4375 13.0625 23.0652 13.0625 20C13.0625 16.9348 15.4348 14.5625 18.5 14.5625C21.5652 14.5625 23.9375 16.9348 23.9375 20C23.9375 23.0652 21.5652 25.4375 18.5 25.4375ZM24.375 12.1875C23.8425 12.1875 23.4375 11.7825 23.4375 11.25C23.4375 10.7175 23.8425 10.3125 24.375 10.3125C24.9075 10.3125 25.3125 10.7175 25.3125 11.25C25.3125 11.7825 24.9075 12.1875 24.375 12.1875Z" fill="white" />
                                        <path d="M18.5 16.4375C16.506 16.4375 14.9375 18.006 14.9375 20C14.9375 21.994 16.506 23.5625 18.5 23.5625C20.494 23.5625 22.0625 21.994 22.0625 20C22.0625 18.006 20.494 16.4375 18.5 16.4375Z" fill="white" />
                                        <defs>
                                            <linearGradient id="paint0_linear_5557_2600" x1="18.5" y1="0" x2="18.5" y2="37" gradientUnits="userSpaceOnUse">
                                                <stop stop-color="#F58529" />
                                                <stop offset="0.5" stop-color="#DD2A7B" />
                                                <stop offset="1" stop-color="#833AB4" />
                                            </linearGradient>
                                            <clipPath id="clip0_5557_2600">
                                                <rect width="37" height="37" fill="white" />
                                            </clipPath>
                                        </defs>
                                    </g>
                        ...
                        <label htmlFor="googlePlus" className="form-label mb-0 heading-14 label-color">Google Plus <span style={{ color: 'red' }}>*</span></label>
                        <div className="input-group mb-3">
                            <span className="input-group-text p-0" id="googlePlus-addon">
                                <svg width="37" height="37" viewBox="0 0 37 37" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g clip-path="url(#clip0_5557_2598)">
                                        ...
                                        <g opacity="0.2">
                                            ...
                                            <path d="M14.0473 16.4796C14.3151 16.723 14.6315 16.9178 15.021 16.9665C15.6782 17.0638 16.2381 16.723 16.5302 16.0171C16.6763 15.6276 16.7493 15.2138 16.725 14.8244C16.7006 14.1408 16.6519 15.4572 16.5302 15.7737C16.2625 16.4796 15.7026 16.8204 15.021 16.723C14.6559 16.6744 14.3394 16.4796 14.0473 16.2362C13.2927 15.5303 12.9276 14.654 12.8789 13.6559C12.8789 13.7046 12.8789 13.7533 12.8789 13.8263C12.9032 14.8487 13.2684 15.7737 14.0473 16.4796Z" fill="white" />
                        ...
                        {isValidGooglePlus && (
                            <p className="ms-1" style={{ color: 'red', fontSize: '14px', marginTop: '-18px' }}>
                                Valid Google Plus URL is required
                            </p>
                        )}

                        <label htmlFor="linkedin" className="form-label mb-0 heading-14 label-color">LinkedIn <span style={{ color: 'red' }}>*</span></label>
                        <div className="input-group mb-3">
                            <span className="input-group-text p-0" id="linkedin-addon">
                                <svg width="37" height="37" viewBox="0 0 37 37" fill="none" xmlns="http://www.w3.org/2000/svg">
