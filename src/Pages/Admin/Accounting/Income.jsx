...
const Income = () => {
  ...
  const MyIncomeGetAllApi = async () => {
    setLoader(true)
    try {
      const response = await IncomeCategorygetAllApi(searchKey,
        pageNo,
        pageSize);
      // console.log('income categoryyyyyy',response)
  ...
  const MyIncomePostApi = async () => {
    if (FuncValidation()) {
      ...
      formData.append('incomeCategoryId', examTermId);
  ...
  const MyIncome2GetAllApi = async () => {
    setLoader(true)
    try {
      const response = await IncomeAllApi(startDate, endDate, examTermId, searchKey, pageNo, pageSize);
      // console.log('get all by date and category search',response)
      if (response?.status === 200) {
        // toast.success(response?.data?.message)
        setMyIncomeCategoryData(response?.data?.incomes)
  ...
  const MyIncomeGetById = async (id) => {
    ...
    try {
      ...
      // console.log('get by id data in income',response)
      if (response?.status === 200) {
        // toast.success(response?.data?.msg)

        setAmount(response?.data?.income?.amount)
        setCategoryId(response?.data?.income?.incomeCategoryId)
        setDate(response?.data?.income?.incomeDate)
        setLoader(false)
            ...
            <table className="table table-sm table-striped">
              <thead className=''>
                <tr className='heading-16 text-color-000' style={{ fontWeight: '500' }}>
                  ...
                  <th className='no-wrap'> Income category</th>
              ...
              <tbody className='heading-14 align-middle greyTextColor'>
                {
                  myIncomeCategoryData?.map((item, index) => (
                    <tr className='heading-14' >
                      ...
                      <td className=' greyText no-wrap' >{item.incomeDate}</td>
