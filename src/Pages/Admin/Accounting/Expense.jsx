...
const Expense = () => {
  ...
  const [examTermId, setCategoryId] = useState()
  ...
  const [isValidBillNoRequired, setIsValidBillNoRequired] = useState(false);
  ...
  const FuncValidation = () => {
    ...
    // marks 
    if (!amount || amount === "" || !/^[a-zA-Z0-9!@#$%^&*\-()_+=\s-]+$/.test(amount)) {
    ...
    // billNo 
    if (!billNo || billNo === "" || !/^[a-zA-Z0-9!@#$%^&*\-()_+=\s-]+$/.test(billNo)) {
      setIsValidBillNoRequired(true)
      setLoader(false)
      isValid = false
    }
    else {
  ...
  // Bill no 
  const handleBiilNo = (e2) => {
    setBillNo(e2);
    const noRegex = /^[a-zA-Z0-9!@#$%^&*\-()_+=\s-]+$/;
    setIsValidBillNoRequired(noRegex.test(e2));
    if (e2 === "" || !noRegex.test(e2)) {
      setIsValidBillNoRequired(true)
    } else {
      setIsValidBillNoRequired(false)
    }
  }
  // date 
  const handleDate = (e2) => {
  ...
  const MyIncomePostApi = async () => {
    ...
    if (FuncValidation()) {
      ...
      formData.append('expenseCategoryId', examTermId);
      formData.append('date', date);
      formData.append('billNo', billNo);
      formData.append('expensePaymentType', expensePaymentType);
  ...
  const MyExpenseGetById = async (id) => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        setCategoryId(response?.data?.expense?.expenseCategoryId)
        setDate(response?.data?.expense?.expenseDate)
        setExpensePaymentType(response?.data?.expense?.expensePaymentType)
        setBillNo(response?.data?.expense?.billNo)
        setLoader(false)
          ...
          <div className='d-flex g-1 for-media-query'>
            <div className='me-2 search-responsive'>
              <div className="input-group mb-3 ">
                ...
                <span className="input-group-text button-bg-color button-color heading-14 font-color " style={{ cursor: 'pointer', height: "34px" }} id="basic-addon2" onClick={MyExpense2GetAllApi}>Search</span>
              </div>
            </div>
            ...
            <table className="table table-sm table-striped">
              <thead className=''>
                <tr className='heading-16 text-color-000' style={{ fontWeight: '500' }}>
                  <th className='no-wrap' style={{ width: '4%' }}>#</th>
                  <th className='no-wrap' style={{ width: '22%' }}>Date</th>
                  <th className='no-wrap' style={{ width: '22%' }}>Amount</th>
                  <th className='no-wrap' style={{ width: '30%' }}>Expense Category</th>
                  <th className='no-wrap' style={{ width: '12%' }}>Action</th>
                </tr>
              </thead>
...

                {
                  myExpenseCategoryData?.map((item, index) => (
                    <tr className='heading-14' >
                      ...
                      <td className=' greyText no-wrap' >{item.expenseDate}</td>
                      <td className=' greyText no-wrap' >{item.amount}</td>
                      <td className=' greyText no-wrap' >{item.expenseCategoryName}</td>
              ...
              <div className="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight" aria-labelledby="offcanvasRightLabel" ref={offcanvasRef}>
                ...
                <div className="offcanvas-body pt-0">
                  <div class="mb-3">
                    ...
                    <input type="date" class="form-control form-control-sm" value={date} onChange={(e) => handleDate(e.target.value)} id="exampleFormControlInput1" placeholder="Select Title" />
                  </div>
                  <div className='pt-1'>
                    {isValidDateRequired && (
                  ...
                  <div class="mb-3">
                    ...
                    <input type="text" class="form-control form-control-sm" value={amount} onChange={(e) => handleAmount(e.target.value)} id="exampleFormControlInput1" placeholder="Enter Amount" />
                  </div>
                  <div className='pt-1'>
                  ...
                  <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label heading-16">Bill No</label>
                    <input type="text" class="form-control form-control-sm" value={billNo} onChange={(e) => handleBiilNo(e.target.value)} id="exampleFormControlInput1" placeholder="Enter Amount" />
                  </div>
                  <div className='pt-1'>
                    {isValidBillNoRequired && (
                      <p className='ms-1' style={{ color: 'red', fontSize: '14px', marginTop: '-18px' }}>
                        Bill no is required
                      </p>
                    )}
                  ...
                  <div className="mb-1  ">
                    <label for="exampleFormControlInput1" className="form-label   heading-16">Expense Payment Type</label>
                    <select class="form-select  form-select-sm form-focus label-color" value={expensePaymentType} onChange={(e) => setExpensePaymentType(e.target.value)} aria-label="Default select example" >
                  ...
                  <div className='my-button11 '>
                    <button type="button" className="btn btn-outline-success heading-16 btn-bgAndColor" style={{ backgroundColor: '#008479', color: '#fff' }} onClick={(e) => MyIncomePostApi()}>Add Expense</button>
            ...
            <>
              <div className="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight1234" aria-labelledby="offcanvasRightLabel" ref={offcanvasRef22}>
                ...
                <div className="offcanvas-body pt-0">
                  ...
                  <div className='my-button11 '>
                    <button type="button" className="btn btn-outline-success heading-16 btn-bgAndColor" style={{ backgroundColor: '#008479', color: '#fff' }} onClick={MyIncomeCategoryPutApi}>Update Expense</button>
