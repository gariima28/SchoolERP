...
//   const watchRoleAdd = addWatch('role');
//   const watchRoleEdit = editWatch('role');
...
//   useEffect(() => {
//     if (watchRoleAdd && typeof watchRoleAdd === 'string' && watchRoleAdd.trim() !== '') {
//       getAllDataByRoleId('add');
//     }
//   }, [watchRoleAdd]);

//   useEffect(() => {
//     if (watchRoleEdit && typeof watchRoleEdit === 'string' && watchRoleEdit.trim() !== '') {
//       getAllDataByRoleId('edit');
//     }
//   }, [watchRoleEdit]);
...
//         setValueUpdate('warehouseName', data.warehouseName || '');
//         setValueUpdate('role', data.roleType || '');
//         setRoleIdEdit(data.roleType || '');
//         setValueUpdate('email', data.keeperEmail || '');
//         setValueUpdate('phoneNumber', data.keeperPhone || '');
//         setValueUpdate('address', data.keeperAddress || '');
//         setValueUpdate('description', data.description || '');
//         setInitialFormValues({ ...data });
//         setViewWarehouse(data);
//         await getAllDataByRoleId('edit'); // Wait for staff data
//         const selectedStaff = DataByRoleId.find(staff => String(staff.id) === String(data.warehouseKeeperId));
//         if (selectedStaff) {
//           setValueUpdate('warehouseKeeper', selectedStaff.id);
//           setValueUpdate('warehouseKeeperName', selectedStaff.staffName);
//         }
//       } else {
//         toast.error(response?.data?.message || 'Failed to fetch Warehouse');
//       }
...
//   const getAllDataByRoleId = async (context = 'add') => {
//     try {
//       setLoaderState(true);
//       const roleId = context === 'add' ? watchRoleAdd : roleIdEdit;
//       if (!roleId) return;
//       const response = await getDataByRoleIdApi(roleId, '', '', '');
//       if (response?.status === 200) {
//         if (response?.data?.status === 'success') {
...
//           setDataByRoleId(staffData);
//           console.log('DataByRoleId:', staffData);
//           if (context === 'edit' && editWarehouseId) {
//             const selectedStaff = staffData.find(staff => String(staff.id) === String(initialFormValues.warehouseKeeperId));
//             if (selectedStaff) {
//               setValueUpdate('warehouseKeeper', selectedStaff.id);
//               setValueUpdate('warehouseKeeperName', selectedStaff.staffName);
//             }
...
//   useEffect(() => {
//     offcanvasRef.current = document.getElementById('Edit_staticBackdrop');
//   }, []);
...
//       offcanvas = new bootstrap.Offcanvas(offcanvasElement, { backdrop: true });
//       offcanvas.show();
//     } else {
//       console.error('Offcanvas element not found: #Edit_staticBackdrop');
//     }
//   };
...
//           if (offcanvas) {
//             offcanvas.hide(); // Hide without disposing immediately
//           }
//           // Remove backdrop after hide completes
//           offcanvasElement.addEventListener('hidden.bs.offcanvas', () => {
//             const backdrop = document.querySelector('.offcanvas-backdrop');
//             if (backdrop) backdrop.remove();
//           }, { once: true });
...
//                         <td className='textWrapClass greyText font14'>{item.keeperAddress}</td>
//                         <td className='text-end'>
//                           <span className="ps-4 greyText" data-bs-toggle="modal" data-bs-target="#viewDetails" style={{ cursor: "pointer" }} onClick={() => getWarehouseDataById(item.id)}><RemoveRedEyeOutlinedIcon /></span>
//                           <span className="ps-4 greyText" data-bs-toggle="offcanvas" data-bs-target="#Edit_staticBackdrop" aria-controls="Edit_staticBackdrop" style={{ cursor: "pointer" }} onClick={() => getWarehouseDataById(item.id)}><DriveFileRenameOutlineOutlinedIcon /></span>
//                           <span className="ps-4 greyText" data-bs-toggle="offcanvas" data-bs-target="#Delete_staticBackdrop" aria-controls="Delete_staticBackdrop" style={{ cursor: "pointer" }} onClick={() => setDelWarehouseId(item.id)}><DeleteOutlinedIcon /></span>
//                         </td>
//                       </tr>
...
//                   ) : (
//                     <option value="" disabled>
//                       {watchRoleAdd ? '-- No Staff Found --' : '-- Select Role First --'}
//                     </option>
//                   )}
//                 </select>
//                 {errorsAdd.warehouseKeeper && <p className="font12 text-danger">{errorsAdd.warehouseKeeper.message}</p>}
...
//                     setValueUpdate('warehouseKeeper', ''); // Reset keeper on role change
//                     getAllDataByRoleId('edit'); // Refresh staff data
//                   }}
//                 >
//                   <option value="">--- Choose ---</option>
//                   {RolesData?.map((role) => (
...
//                   {DataByRoleId.length > 0 ? (
//                     DataByRoleId.map((staff) => (
//                       <option key={staff.id} value={staff.id} selected={String(staff.id) === String(initialFormValues.warehouseKeeperId)}>
//                         {staff.staffName}
//                       </option>
//                     ))
//                   ) : (
//                     <option value="" disabled>
//                       {watchRoleEdit ? '-- No Staff Found --' : '-- Select Role First --'}
//                     </option>
//                   )}
//                 </select>
//                 {errorsEdit.warehouseKeeper && <p className="font12 text-danger">{errorsEdit.warehouseKeeper.message}</p>}
...
//                     const offcanvas = bootstrap.Offcanvas.getInstance(offcanvasElement);
//                     if (offcanvas) {
//                       offcanvas._config.backdrop = true; // Ensure backdrop is restored
//                       offcanvas.hide();
//                     }
//                   }}
//                 >
