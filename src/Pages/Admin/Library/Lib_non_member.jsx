...
import { GetAllNonMemberApi } from '../../../Utils/Apis'
import { AddLibraryMemberApi } from '../../../Utils/Apis'
...
const Lib_non_member = ({ data }) => {
  ...
  const [nonMember, setNonMember] = useState('NON_LIBRARY_MEMBER')

  const [AllnonMember, setAllNonMember] = useState([]);
  const [removeID, setRemoveID] = useState();
  console.log('student id for remove', removeID)

  useEffect(() => {
    // MyStudentGetApi(data)
    MyAddRemoveLibrarianGetAllApi()
  }, [data])
  ...
  const MyAddRemoveLibrarianGetAllApi = async () => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        setAllNonMember(response?.data?.Members)
        setLoader(false)
      } else {
  ...
  // Add Library Member Api
  const RemoveMemberApi = async () => {
    setLoader(true)
    try {
      const response = await AddLibraryMemberApi(removeID);
          ...
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content text-center p-3">
              ...
              <div className=''>
                ...
                <div>
                  <h4>Are you sure to want add this member</h4>
                </div>
