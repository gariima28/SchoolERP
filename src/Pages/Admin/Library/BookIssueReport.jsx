...
import Connect_mb_nmb from './Connect_mb_nmb';
...
const BookIssueReport = () => {
  ...
  // console.log('my returnnnnn date', returndate)
  ...
  const FuncValidation = () => {
    let isValid = true
    // for date
    if (!returndate || returndate === "" || !/^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$/.test(returndate)) {
  ...
  const MySectionGetApi = async () => {
    ...
    try {
      ...
      console.log('section-get-all-api in Book issue report ', response)
      if (response?.status === 200) {
  ...
  const MyStudentGetApi = async () => {
    ...
    try {
      ...
      console.log('student-get-all-api in Book issue report ', response)
      if (response?.status === 200) {
        // toast.success(response?.data?.message)
  ...
  const MyBookIssueGetApi = async () => {
    ...
    try {
      ...
      console.log('data get alllll', response)
      if (response?.status === 200) {
        // toast.success(response?.data?.msg)
  ...
  const MyIssueBookGetByIdApi = async (id) => {
    ...
    try {
      ...
      console.log('my get by id data0101010', response)
      ...
      setStudent(response?.data?.Transaction?.studentId)
  ...
  return (
    <Container>
      ...
      <div className="container-fluid main-body p-3">
          ...
          <div className="breadCrum ms-2">
            <nav style={{ '--bs-breadcrumb-divider': "'>'" }} aria-label="breadcrumb">
              <ol className="breadcrumb ms-2">
                ...
                <li className="breadcrumb-item active heading-14 font-color" aria-current="page">Library</li>
                <li className="breadcrumb-item breadcrum-li heading-14" ><Link href="#">Library Member</Link></li>
              </ol>
            </nav>
          </div>
        ...
        <h5 className='ms-3 mb-2 margin-minus22 heading-16' style={{ marginTop: '-12px' }}>Library Member</h5>

        <div className="main-content-conatainer  pt-1 ">
          {/* ###### copy content till here for all component ######  */}

          <div className="row p-4 d-flex justify-content-center mb-3">
            ...
            <table className="table table-sm table-striped ">
              <thead className='' style={{ borderTop: '1px solid #DDDDEB' }}>
                <tr className='heading-16 text-color-000' style={{ fontWeight: '500' }}>
                  ...
                  <th className='no-wrap ' >Issue Date</th>

                  <th className='no-wrap ' >Return Date</th>
                  <th className='no-wrap ' >Return Delay</th>
                  ...
                  issuesData && issuesData?.length > 0 ? (
                    issuesData?.map((item, index) => (
                      <tr className='heading-14' key={item.transactionId}>
                        ...
                        <td className=' greyText pe-0 no-wrap '>{item.issueDate}</td>
                        <td className=' greyText pe-0 no-wrap  my-anchor-view'>{item.returnDate}</td>
              ...
              <>
                ...
                <div class="offcanvas-body">
                  <div className="input " >
                    ...
                    <div className="mb-1  ">
                      <label for="exampleFormControlInput1" className="form-label  heading-16">Class</label>
                      <select class="form-select  form-select-sm form-focus  label-color" value={''} onChange={(e) => handle(e)} aria-label="Default select example">
                        {
                          classData?.map(item => (
                            <option value={`${item.classId}, ${item.classNo}`}>{item.classNo}</option>
                          ))
