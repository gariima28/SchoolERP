...
const ProfilePage = () => {
        ...
        try {
            ...
            if (response?.status === 200) {
                if (response?.data?.status === 'success') {
                    ...
                    setStudentName(response?.data?.student?.fatherName)
                    ...
                    setValue('phoneNumber', response?.data?.student?.parentNo)
                    setStudentAddress(response?.data?.student?.address)
                    setValue('studentAddress', response?.data?.student?.address)
                    setStudentEmail(response?.data?.student?.parentEmail)
                    setStudentDOB(response?.data?.student?.dateOfBirth)
                    setStudentGender(response?.data?.student?.studentGender)
                    setValue('multipartFile', response?.data?.student?.parentImage)
                    setSchoolLogoVal(response?.data?.student?.parentImage)
                    // toast.success(response.data.message);
                    if (response?.data?.student?.parentImage) {
    ...
    const updateProfileData = async (data) => {
        try {
            setloaderState(true);
            // console.log(changeImageType)
            const formData = new FormData();
            if (StudentAddress !== data.studentAddress) {
                ...
                <div className="col-md-8 col-sm-12 px-md-3 p-1">
                    <form className="row mb-1 g-3" onSubmit={handleSubmit(updateProfileData)}>
                        <div className="col-12">
                            ...
                            <input type="text" className={`form-control font14 readonly-bg`} id="validationDefault02" value={StudentName} disabled />
                        </div>
                        <div className="col-12">
                            <label htmlFor="validationDefault02" className="form-label font14">Email</label>
                            <input type="text" className={`form-control font14 readonly-bg`} id="validationDefault02" value={StudentEmail} disabled />
                        </div>
                        {/* <div className="col-12">
